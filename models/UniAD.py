...
class UniAD(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        assert isinstance(inplanes, list) and len(inplanes) == 2
        ...
        self.output_proj = nn.Linear(hidden_dim, inplanes[1])
    ...
    def getEncoderOutput(self, input):
        feature_align = input  # B x C X H x W
        feature_tokens = rearrange(
            feature_align, "b c h w -> (h w) b c"
        )  # (H x W) x B x C
        feature_tokens = self.input_proj(feature_tokens)  # (H x W) x B x C
        pos_embed = self.pos_embed(feature_tokens)  # (H x W) x C
        _, output_encoder = self.transformer(
            feature_tokens, pos_embed
        )  # (H x W) x B x C
        output_encoder = rearrange(
            output_encoder, "(h w) b c -> b c h w", h=self.feature_size[0]
        )  # B x C X H x W
...
def getUniAD():
    ...
    config.net[2].kwargs.feature_size = [32, 32]
    # print(len(config.net))
    # model = ModelHelper(config.net)
    cfg_subnet = config.net[2]
    kwargs = cfg_subnet["kwargs"]
    kwargs["inplanes"] = [256, 240]
    model = UniAD(**kwargs)
    # print(model)
    return model

if __name__ == '__main__':
    data = torch.ones((2, 256, 32, 32)).cuda()
    model = getUniAD().cuda()
    feature_rec = model(data)
    print(feature_rec.shape)
    encode = model.getEncoderOutput(data)
    print(encode.shape)
